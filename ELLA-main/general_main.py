...
def main(args):
    ...
    if args.randomize_seed:
        r_seed = random.randint(0, args.seed)
    else:
        r_seed = args.seed
...
if __name__ == "__main__":
    ...
    #----------------------hyperparameters for ELLA------------------------------------#
    parser.add_argument('--ELLA_alpha', dest='ELLA_alpha', default=1, type=float,
                        help='Factor for the Equalization Loss at stage1 of ELLA Loss_stage1 = Loss_contrastive + alpha * Loss_equalization')
    parser.add_argument('--ELLA_beta', dest='ELLA_beta', default=0.1, type=float,
                        help='Beta controls the maximum scale of the feature augmentations')
    
    ########################General###################################################
    parser.add_argument('--write_file', dest='write_file', default=True, type=boolean_string)
    parser.add_argument('--file_name', dest='file_name', default=None, type=str)
    parser.add_argument('--num_runs', dest='num_runs', default=1, type=int,
                        help='Number of runs (default: %(default)s)')
    parser.add_argument('--cuda_device', dest='cuda_device', default = 0, type=int, help='choose the gpu to run')
    parser.add_argument('--seed', dest='seed', default=1993, type=int,
                        help='Random seed')
    parser.add_argument('--randomize_seed', dest='randomize_seed', default=True, type=boolean_string)
    parser.add_argument('--stage2_activate', dest='stage2_activate', default=False, type=boolean_string, help='Activate stage 2 in Delta')
    ########################Data Distribution#########################
    parser.add_argument('--distribution', dest='distribution', default='conv', choices=['conv','exp','fewshot', 'step'], type=str,
                        help='Choice of data distribution within Longtail/ Ordered Longtail (default: %(default)s)')
    parser.add_argument('--nc_first_task', dest='nc_first_task', default=5, type=int, help='Choice of different number of classes in the first phase')
    parser.add_argument('--lt', dest='lt', default=False, help='Setting (high level) Long tailed distribution for data')
    parser.add_argument('--ltio', dest='ltio', default=False, help='Setting (high level) Ordered Long tailed distribution for data')
    parser.add_argument('--imb_factor', dest='imb_factor', default=0.01, help='Setting the imbalance factor for long tail distributions')
    ...
    parser.add_argument('--agent', dest='agent', default='ER',
                        choices=['ER', 'EWC', 'AGEM', 'CNDPM', 'LWF', 'ICARL', 'GDUMB', 'ASER', 'SCR','DELTA','ELLA'],
