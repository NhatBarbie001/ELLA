...
class ImageNet_Subset(DatasetBase):
	def _get_img_from_paths(self, text_file):
		...
		with open(text_file,'rb') as f:
			for line in f:
				...
				temp = temp.split('/')
				end_part = temp[-1:]
				temp = '../../ImageNet2012/'+temp[2]+"/"+temp[3]+"/"+str(end_part[0].split(' ')[0])
				data.append(temp)
	...
	def __init__(self, scenario, params):
		...
		self.train_file = '../../ImageNet2012/train_1993_100.txt'
		self.test_file = '../../ImageNet2012/test_1993_100.txt'
	...
	def setup(self):
		if self.scenario == 'nc':
			...
			self.task_labels, self.data = create_task_composition(class_nums=100, num_tasks=self.task_nums, nc_first_task=self.params.nc_first_task, class_order = class_order_table[class_order_var], \
                                                                    x=self.train_data, y=self.train_label, x_test=self.test_data, y_test=self.test_label, lt=self.params.lt, ltio=self.params.ltio, fixed_order=self.params.fix_order, imb_factor=self.params.imb_factor, dataset_flag='imagenet_subset')
			self.test_set = []
