...
class TrainLearner(object):
    ...
    def test_model(self, loader, i):
        ...
        # (Your existing code to evaluate the model)
        ...
        for batch_idx, (data, target) in enumerate(loader):
            ...
            all_preds.extend(Pred.cpu().numpy().flatten())  # Flatten to convert 2D to 1D
        ...
        sns.heatmap(conf_matrix, annot=False, fmt='d', cmap='rocket') #, xticklabels=xtixks, yticklabels=xtixks)
        plt.ylabel('True Label')
        plt.xlabel('Predicted Label')
        plt.savefig(os.path.join('plots', f'CM_{i}_onpro.png' ), bbox_inches='tight', dpi=300)
        plt.close()
