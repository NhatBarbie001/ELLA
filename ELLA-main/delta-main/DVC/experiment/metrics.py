...
def compute_performance(end_task_acc_arr, params):
    """
    ...
    print('\n')
    print_accuracy = (np.sum(np.tril(end_task_acc_arr), axis=2) /(np.arange(n_tasks) + 1)).tolist()
    print('actual accuracy phase wise: ', print_accuracy)

    #write values into a file - change each run
    if params.write_file:
        paths = 'dvc_lfh.txt'

        print('text file path: ',paths)
        if params.mem_size is None:
            settings_str = str(params.agent) + " "+ str(params.data)+" "+str(params.update)+" "+str(params.retrieve)+" "+" LT:  "+str(params.lt)+ " LTIO: "+str(params.ltio)+' LFH: '+str(params.lfh)
        elif params.mem_size is not None:
            if params.update is not None:
                settings_str = str(params.agent) + " " + str(params.mem_size) + " "+ str(params.data)+" "+str(params.update)+" "+str(params.retrieve)+" "+" LT:  "+str(params.lt)+ " LTIO: "+str(params.ltio)+' LFH: '+str(params.lfh)
            elif params.update is None:
                settings_str = str(params.agent) + " " + str(params.mem_size) + " "+ str(params.data)+" LT:  "+str(params.lt)+ " LTIO: "+str(params.ltio)+' LFH: '+str(params.lfh)

        
        with open(paths, 'a') as outfile:
            json.dump(settings_str, outfile)
            outfile.write('\n')
            json.dump(print_accuracy, outfile) #accuracy phase wise
            outfile.write('\n')
            json.dump(str(avg_fgt), outfile) #forgetting
            outfile.write('\n')
