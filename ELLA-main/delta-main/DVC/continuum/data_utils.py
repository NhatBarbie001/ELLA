...
from utils.setup_elements import transforms_match, class_distribution_table, class_distribution_table_imagenet
...
#############################################################
#########___________ TASK CREATE_____________##############
#############################################################

def create_task_composition(class_nums, num_tasks, nc_first_task, class_order, \
                            x, y, x_test, y_test, lt=False, ltio=False, fixed_order=True, imb_factor=0.01, lfh=False, dataset_flag = 'imagenet_subset'):
    ...
    print('lfh: ', lfh)
    if lfh:
        nc_first_task = int(class_nums / 2)
        num_tasks += 1
...
#############################################################
#########__________VFN TASK CREATE_____________##############
#############################################################
...
def create_task_composition_vfn(class_nums, num_tasks, nc_first_task, class_order, \
    ...
    # for i in range(num_tasks):
    #     print('training labels in task', i, ': ')
    #     unique(data[i]['trn']['y'])
    #     print('test labels in task', i, ': ')
    #     unique(data[i]['tst']['y'])
    #     print('\n')
