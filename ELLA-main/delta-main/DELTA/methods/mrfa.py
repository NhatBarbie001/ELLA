...
class MRFA:
    ...
    def feature_augmentation(self, model, samples, samples_aug, targets, net_type, criterion=None, for_mem_x_aug = False):
    ...
    def register_perturb_forward_prehook(self, model, net_type):
        if net_type == 'resnet32':
            ...
            self.register_perturb_forward_prehook_layers(model, model.encoder, 5, register_forward_prehook_resnet18)
        else:
            raise ValueError(f'Unknown net_type {net_type}.')
    ...
    def get_feature_augmentation(self, model, convnet, samples, samples_aug, targets, num_layers, register_func, criterion=None, for_mem_x_aug = False):
        if for_mem_x_aug:
            #print("\n=== Computing Gradient/Perturbations for augmented samples ===")
            #samples.requires_grad_(True)
            features_orig = model.forward(samples).unsqueeze(1)
            #print(f"features shape of model.forward(samples): {features_orig.shape}")
            ...
            inp_grads = [inp.grad.detach().clone() for inp in layer_inputs]
            # print("\nGradient shapes for each layer:")
            # for i, grad in enumerate(inp_grads):
            #     print(f"Layer {i} gradient shape: {grad.shape}, norm: {grad.norm().item():.4f}")
            
            samples.requires_grad_(False)
            samples_aug.requires_grad_(False)

            self.perturbations = inp_grads
            #print(f"\nStored {len(self.perturbations)} perturbations")
            for i, p in enumerate(self.perturbations):
                if torch.isnan(p).any():
                    print(f"WARNING: NaN values detected in perturbation {i}!")
                    raise ValueError()
                print(f"Perturbation {i} shape: {p.shape}")
        else:
            #print("\n=== Debug for original samples ===")
            #samples_aug.requires_grad_(True)
            features_aug = model.forward(samples_aug).unsqueeze(1)
            #print(f"features shape of : model.forward(samples_aug) {features_aug.shape}")
            ...
            samples.requires_grad_(True)
            features_orig = model.forward(samples).unsqueeze(1)
            #print(f"Original features shape: {features_orig.shape}")
            ...
            for p in self.perturbations:
                if torch.isnan(p).any():
                    print("WARNING: NaN values detected in perturbations!")
                    raise ValueError()
    ...
    def register_perturb_forward_prehook_layers(self, model, convnet, num_layers, register_func):
        def perturb_input_prehook_full(module: torch.nn.Module, inp, layer_id):
            print(f"\n=== Debug Applying Hook While training ===")
            # print(f"Layer ID: {layer_id}")
            # print(f"Number of perturbations: {len(self.perturbations)}")
            # print(f"Perturbation layers: {self.perturbation_layers}")
            # print(f"Perturbation indices: {self.perturbation_idices}")
            # print(f"Perturbation indices in batch: {self.perturbation_idices_inbatch}")
            ...
            if layer_id in self.perturbation_layers:
                ...
                #print(f"Perturbation index for layer {layer_id}: {p_index}")
                ...
                #print(f"Perturbation shape for layer {layer_id}: {self.perturbations[layer_id].shape}")
...
def register_forward_prehook_resnet18(model, convnet, hooks):
    ...
    # Đăng ký hook cho linear layer (thay vì fc)
