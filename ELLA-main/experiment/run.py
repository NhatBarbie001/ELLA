...
def multiple_run(params, store=False, save_path=None):
    my_device = 'cuda:'+str(params.cuda_device)
    torch.cuda.set_device(my_device)
    print('params cuda: ', params.cuda, torch.cuda.current_device())
    # Set up data stream
    start = time.time()
    ...
    if store:
        ...
        save_file.close()
    if params.online:
        avg_end_acc, avg_end_fgt, avg_fgt, avg_acc, avg_bwtp, avg_fwt = compute_performance(accuracy_array, params, (run_end-run_start))
        print('----------- Total {} run: {}s -----------'.format(params.num_runs, end - start))
        print('----------- Avg_End_Acc {} Avg_End_Fgt {} Avg_Fgt {} Avg_Acc {} Avg_Bwtp {} Avg_Fwt {}-----------'
              .format(avg_end_acc, avg_end_fgt, avg_fgt, avg_acc, avg_bwtp, avg_fwt))
    else:
        print('----------- Total {} run: {}s -----------'.format(params.num_runs, end - start))
